...
- **CRM-166 — the `agent` role reads custom attribute definitions** (migration `20260818120001`) — the default `agent` seed grants `custom_attribute_definitions.read`. The role already wrote attribute VALUES (`conversations.custom_attributes`, `contacts.destroy_custom_attributes`) but held no read on the DEFINITIONS, which is what `Api::V1::CustomAttributeDefinitionsController#index` gates on in the CRM: the 403 surfaced in the UI as "no custom attributes", so a contact's attributes were only visible inside the edit form (which falls back to rendering raw keys). Only the read is granted — `create`/`update`/`delete` on a definition stay administrative. Note the side effect: the CRM gates the Settings › Custom Attributes menu entry and route on the same `read` key, so the agent now reaches that screen in read-only mode. An idempotent data-migration backfills the key on already-bootstrapped installations (fresh installs pick it up from the seed).
