...
- **CRM-166 — the `agent` role reads custom attribute definitions** (migration `20260818120005`) — the default `agent` seed grants `custom_attribute_definitions.read`. The role already wrote attribute VALUES (`conversations.custom_attributes`, `contacts.destroy_custom_attributes`) but held no read on the DEFINITIONS, which is what `Api::V1::CustomAttributeDefinitionsController#index` gates on in the CRM: the 403 surfaced in the UI as "no custom attributes", so a contact's attributes were only visible inside the edit form (which falls back to rendering raw keys). Only the read is granted — `create`/`update`/`delete` on a definition stay administrative. The CRM used to gate the Settings › Custom Attributes menu entry and route on this same `read` key, which would have handed the agent that screen; **deploy together with the evo-ai-frontend-community change that moves those two gates to the administrative `create`/`update`/`delete` keys**, or the agent reaches Settings in the window between the two deploys. An idempotent data-migration backfills the key on already-bootstrapped installations (fresh installs pick it up from the seed).

- **CRM-70 — `manage` action on `macros`, `message_templates` and `teams`: use-vs-manage split** (migrations `20260818120001` + `20260818120003`) — the attendance role USES these in the chat (`macros.read`/`macros.execute`, `message_templates.read`, `teams.read` for the assignment picker) but no longer MANAGES them: `create`/`update` and the Settings screens move to a standalone `<resource>.manage`, the same shape as `users.manage` (registered in `STANDALONE_ACTIONS_BY_RESOURCE`, so it stays out of the role editor's coarse Write group and adds no `write` leaf). Product decision 2026-08-18: `labels` and `canned_responses` are NOT part of the split — the agent keeps managing its own. `GrantManageActionsToManagingRoles` backfills the new keys to the admin roles AND to every role, system or custom, that can already manage the resource today (`macros`/`message_templates`: the create/update keys; `teams`: a team write, deliberately NOT `teams.read`, which is in `User::BASIC_READ_PERMISSIONS` and would hand the key to everyone) — without it the gate split would take management away from those roles silently on upgrade; granting it preserves their reach, it is not an expansion. The system `agent` role is excluded from that backfill, and `RevokeManageWritesFromAgent` then drops `macros.create/update` and `message_templates.create/update` from it (custom roles untouched; rollback forward-only). Deploy together with the evo-ai-crm-community change that gates `MacrosController`/`MessageTemplatesController` create+update (and `MessageTemplatePolicy`) by `<resource>.manage`, and with both frontends. Catalog grows 329→332 keys / 51 resources (unchanged).

  **Operator action:** a custom role that today manages macros or message templates keeps its reach automatically, but `macros.create/update` and `message_templates.create/update` become orphan keys — no controller consumes them. Grant management from the role editor's standalone **Manage** row; the coarse **Write** group no longer opens those screens.

- **CRM-178 — `pipeline_items.update` card-write permission** (migration `20260817120009`) — new RBAC resource `pipeline_items` (single `update` action + coarse `write` leaf) and its grant to the default `agent` role, so moving/creating/editing pipeline cards gates on a dedicated key instead of the manager-level `pipelines.update`. The data-migration also backfills the new key to every role — system OR custom — that already holds `pipelines.update`: those roles can write cards today, and the gate split would otherwise take it away from them silently on upgrade (granting it preserves their exact reach, it is not an expansion). Deploy together with the evo-ai-crm-community change that gates `PipelineItemsController` by `pipeline_items.update` (`PipelinePolicy#update_items?`). Catalog grows 327→329 keys / 50→51 resources.

### Changed

- **CRM-535 — `GET /api/v1/users` returns a directory payload** — the listing, and `POST`/`PATCH /api/v1/users` when the target is not the caller, serialize with the new `UserSerializer.directory` (id, name, email, type, role, avatar, display/available name, availability, confirmed, timestamps). `UserSerializer.for_reader` keeps the full payload only when the target is the caller itself. **Breaking for integrators:** `pubsub_token`, `ui_settings`, `custom_attributes`, `mfa_enabled` and `mfa_setup_incomplete` no longer appear in those three responses for anyone but the user themselves. Login, `/api/v1/auth/me`, `/api/v1/auth/validate` and registration are the user's own payloads and are unchanged. The unused `UserSerializer.list` variant is folded into `directory`.

- **CRM-190 — least-privilege agent: revoke shared-asset deletes** (migration `20260817120007`) — follow-up to CRM-182 for four keys outside its enumerated list. The default `agent` seed no longer grants `labels.delete`, `macros.delete`, `canned_responses.delete`, or `message_templates.delete` — deleting a shared, account-wide asset (a deleted label drops off every conversation) is a manager action, not attendance. `read`/`create`/`update` and `macros.execute` stay. `macros.delete` carries a CRM-side carve-out: `Macro#set_visibility` forces `personal` for every non-admin, so a macro an agent creates belongs to that agent alone and its owner may still delete it without the key — the revoke removes only the power to delete GLOBAL macros. A data-migration revokes the keys from the EXISTING system `agent` role on upgrade (custom roles untouched); rollback is forward-only (re-granting would escalate privilege). The 403/200 enforcement is proven end-to-end in evo-ai-crm-community (`spec/requests/api/v1/agent_shared_asset_deletes_rbac_spec.rb`).
