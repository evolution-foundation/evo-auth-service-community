...
|---|---|---|
| Seed | `db/seeds/rbac.rb` | Grants the whole catalog to `super_admin`, and the catalog minus `account_owner_exclusive` to `account_owner`. **Not fresh-install-only:** the grant blocks sit outside `new_record?`, so every `rails db:seed` rewrites both sets against the running image's catalog. Seeding is the deployment's call — this image does not seed on boot; the Evolution deployments run `db:prepare && db:seed` on every deploy. |
...
| Editing | `Api::V1::RolesController#bulk_update_permissions` | Rejects permission edits on `super_admin` with 403. Its grant set is an invariant, not a preference: accepting the edit would persist it, return 200, and let the next boot revert it silently. The same 403 covers **every `system: true` role**, for the same reason one level down: `db/seeds/rbac.rb` rewrites their permission sets wherever the deploy runs `db:seed`. `super_admin` is additionally matched by key, so its guard does not depend on the flag. Viewing is untouched — `show`/`index` still return the full permission set. |
